...
public class YoutubeDragLayout extends ViewGroup {
	...
	private class DragHelperCallback extends ViewDragHelper.Callback {
		...
		public void onViewPositionChanged(View changedView, int left, int top,
				int dx, int dy) {
			// Log.i(TAG, "onViewPositionChanged:" + "left:" + left + ",top:" +
			// top + ",dx:" + dx + ",dy:" + dy);
			mTop = top;
			mDragOffset = (float) top / mDragRange;

			// Log.i(TAG, "onViewPositionChanged:" + "mDragOffset:" +
			// mDragOffset);
		...
		public void onViewReleased(View releasedChild, float xvel, float yvel) {
			...
			// yvel Fling浜х敓鐨勫�硷紝yvel > 0 鍒欐槸蹇�熷線涓婩ling || yvel < 0
			// 鍒欐槸蹇�熷線涓奆ling

			int top = getPaddingTop();
			if (yvel > 0 || (yvel == 0 && mDragOffset > 0.4f)/*
															 * 鍚庨潰杩欎釜灏忔嫭鍙烽噷鍒ゆ柇澶勭悊鎷栧姩涔嬪悗鍋滀笅鏉ヤ絾鏄
															 * 湭鏉炬墜鐨勬儏鍐�
															 */) {
				top += mDragRange;
			}
			mDragHelper.settleCapturedViewAt(releasedChild.getLeft(), top);
			invalidate();// important 涓嶅姞锛屽氨涓嶄細鍒锋柊View鐨勪綅缃�
							// 灏肩帥銆傘�傘�傘�備綘璇曡瘯娉ㄩ噴鎺夌湅鐪嬨�傚紕姝绘垜浜嗭紝鐫¤銆�
		}
