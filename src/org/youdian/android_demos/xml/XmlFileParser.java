			...
			while(eventType != XmlResourceParser.END_DOCUMENT){
				if(eventType == XmlResourceParser.START_TAG){
					if(parser.getName().equals(XML_TAG_TIMEZONE)){
						String weatherID=parser.getAttributeValue(NAME_SPACE, WEATHER_ID);
						String gmtZone=parser.getAttributeValue(NAME_SPACE, GMT_ZONE);
						String cityName=parser.nextText();
						Log.d(XML_TAG_TIMEZONE, "cityName="+cityName+",weather id="+weatherID+
								",gmtZone="+gmtZone);
					}
				}
