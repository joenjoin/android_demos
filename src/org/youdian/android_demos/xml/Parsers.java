...
public class Parsers {
	...
	public class SAXParser implements Parser<Person>{
		...
		public String serialize(List<Person> list) throws Exception {
			...
			for(Person p:list){
				attrs.clear();
				attrs.addAttribute(uri, localName, "id", "string", String.valueOf(p.getId()));
			...
			for(int i=0;i<nodes.getLength();i++){
				...
				for(int j=0;j<properties.getLength();j++){
					...
					if(nodeName.equals("id")){
						int id=Integer.parseInt(property.getFirstChild().getNodeValue());
						p.setId(id);
					}else if(nodeName.equals("name")){
						String name=property.getFirstChild().getNodeValue();
						p.setName(name);
					}else if(nodeName.equals("age")){
						int age=Integer.parseInt(property.getFirstChild().getNodeValue());
						p.setAge(age);
					}
				}
				list.add(p);
